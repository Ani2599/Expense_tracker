...
async def init_db():
    ...
    This avoids blocking at import time and ensures the DB schema exists
    and is writable before the server starts.
    """
    try:
...
if __name__ == "__main__":
    # Initialize DB asynchronously before starting the server
    try:
        asyncio.run(init_db())
    except Exception:
        # If DB init fails, propagate the error so it is visible to the operator
        raise
